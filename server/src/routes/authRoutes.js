...
router.post("/login", async (req, res, next) => {
  try {
    ...
    const result = await pool.query("SELECT id, email, name, password_hash, active FROM nexus_admin_users WHERE email = $1", [email]);
    ...
    const token = jwt.sign({ sub: admin.id, email: admin.email, role: "nexus_admin" }, env.jwtSecret, { expiresIn: "8h" });
...
router.get("/me", authenticate, async (req, res, next) => {
  try {
    const result = await pool.query("SELECT id, email, name, active FROM nexus_admin_users WHERE id = $1", [req.admin.sub]);
