...
function StudyCodeBillingPanel({ token, onError, onBack }) {
  ...
  return <section className="page-section detail-page billing-page studycode-billing-page">
    ...
    <section className="workspace-panel plan-delete-panel"><div className="section-heading"><div><span className="eyebrow">GERENCIAR PLANOS</span><h2>Excluir plano</h2><p>Exclua somente planos que ainda não possuem assinaturas vinculadas.</p></div></div><div className="action-row">{data.plans.map((plan) => <button className="button-quiet danger-button" key={plan.id} onClick={() => deletePlan(plan)}>Excluir {plan.name}</button>)}</div></section>
  </section>;
}
