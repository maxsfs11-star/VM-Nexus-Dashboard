...
function App() {
  ...
  async function createUnit(event) { event.preventDefault(); setSalvando(true); const form = new FormData(event.currentTarget); try { await criarUnidade(sessao.token, tenantSelecionado.id, { name: form.get("name"), slug: form.get("slug"), city: form.get("city"), state: form.get("state") }); const { units } = await listarUnidades(sessao.token, tenantSelecionado.id); setUnidades(units); await refresh(); event.currentTarget.reset(); } catch (error) { setErro(error.message); } finally { setSalvando(false); } }
