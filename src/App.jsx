...
function ProductForm({ product, onSubmit, onClose, saving }) {
...
function ProductsView({ products, onSave, onDelete, saving }) {
  ...
  return <section className="tenants-view products-view">
    ...
    {editing !== undefined && <ProductForm product={editing} saving={saving} onClose={() => setEditing(undefined)} onSubmit={async (event, product) => { if (await onSave(event, product)) setEditing(undefined); }} />}
  </section>;
}
...
function App() {
  ...
  async function createPlan() { const name = window.prompt("Nome do plano:", "Novo plano"); if (!name) return; try { await criarPlano(sessao.token, { productKey: planProductKey, name, slug: name, description: "Plano personalizado", monthlyPrice: 0 }); await refreshPlans(); } catch (error) { setErro(error.message); } }
