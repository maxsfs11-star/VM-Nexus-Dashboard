...
import { atualizarPagamento, criarPagamento, listarFinanceiro, listarPagamentos, listarTenants } from "./api";
...
export default function FinancialPaymentsView({ token, onError }) {
  ...
  async function load(nextStatus = status) {
    setLoading(true);
    try {
      const [financial, paymentData, tenantData] = await Promise.all([listarFinanceiro(token), listarPagamentos(token, nextStatus), listarTenants(token)]);
      setData(financial);
